...
export const GET = async () => {
    try {
        ...
        return NextResponse.json({message: "Error fetching ratings and reviews",  error: (error as Error).message }, { status: 500 });
    }
};
...
export const POST = async (req: NextRequest) => {
    ...
    try {
        ...
        return NextResponse.json({ message: "Error adding your rating and review", error: (error as Error).message }, { status: 500 });
    }
};
